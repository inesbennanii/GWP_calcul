...
if selected_menu=='Calcul GWP':
    selected = option_menu(
        ...
        icons=['lightning-charge', 'transformer', 'battery-full'],
        menu_icon='cast',
        default_index=0,
    ...
    if selected == 'Cables':
        ...
        if "GWP1" in st.session_state and st.button("Choisir l'aluminium", key="alu"):
            st.session_state.total_liste.append(st.session_state.GWP1)

        if "GWP2" in st.session_state and st.button("Choisir le cuivre",key="cuivre"):
            st.session_state.total_liste.append(st.session_state.GWP2)
        ...
        if st.button("Valider"):
            ...
            st.metric(label="Total GWP du transformeur séléctionné", value=f"{GWP3} kg CO₂")
        if "GWP3" in st.session_state and st.button("Ajouter à la liste"):
            st.session_state.total_liste.append(st.session_state.GWP3)
        ...
        types = {
            ...
            'TH4': 'SM AirSeT - DMVLS - CDTS - MX220 - 20kV - IAC 12,5 kA 1s AFL - verr. C4 - VIP45',
        ...
        for i, cellule in enumerate(st.session_state.cellules):
            cellule['type'] = st.selectbox(
                f"Cellule {i+1} :",
        ...
        if st.session_state.cellules and st.button("Valider"):
            ...
            for cellule in st.session_state.cellules:
                for s in cellule.keys():
                    st.write(f"{cellule[s]}: {types[cellule[s]]}")
        
            st.metric(
                label="Total GWP des cellules",
                value=f"{GWP4} kg CO₂",
            )
        if "GWP4" in st.session_state and st.button("Ajouter à la liste"):
            st.session_state.total_liste.append(st.session_state.GWP4)
    ...
    if total_liste: 
        total = sum(total_liste)
        st.metric(label="Total GWP", value=f"{total} kg CO₂")
        
        st.subheader("Liste des éléments")
        for i, gwp in enumerate(total_liste, 1):
            st.write(f"Élément {i}: {gwp} kg CO₂")
